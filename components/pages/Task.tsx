  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    switch (activeTab) {
      ...
      case "inbox":
        return true;
      case "today":
        return task.reminder && new Date(task.reminder).toDateString() === new Date().toDateString();
      case "upcoming":
        return task.reminder && new Date(task.reminder) > new Date();
      case "important":
    ...
    <div className="flex flex-col h-screen bg-[#fbfdfc] dark:bg-[#1e1e1e] text-[#1b281b] dark:text-white">
      ...
      <div className="md:hidden flex items-center justify-between h-16 px-4 border-b border-[#eef6ef] dark:border-[#2c2c2c] bg-[#3f9142] text-white">
        ...
        <h1 className="text-xl font-medium">
          {activeTab === "inbox" ? "Inbox" : activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}
        </h1>
        <div className="relative">
          <button onClick={toggleProfileDropdown} className="p-2 rounded-md text-white hover:bg-[#357937]">
            <User size={20} />
          </button>
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-2 bg-[#fbfdfc] dark:bg-[#1e1e1e] border-b border-[#eef6ef] dark:border-[#2c2c2c]">
        ...
        <button onClick={toggleTheme} className="p-2 rounded-md text-[#4f4f4f] dark:text-[#bdbdbd] hover:bg-[#eef6ef] dark:hover:bg-[#2f3630]">
          {theme === "dark" ? "☀️" : "🌙"}
        </button>
      </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <header className="hidden md:flex items-center justify-between h-16 px-4 border-b border-[#eef6ef] dark:border-[#2c2c2c] bg-[#3f9142] text-white">
            <div className="flex items-center">
              ...
              <h1 className="ml-4 text-xl font-medium">
                {activeTab === "inbox" ? "Inbox Tasks" : activeTab.charAt(0).toUpperCase() + activeTab.slice(1) + " Tasks"}
              </h1>
            </div>
          ...
          <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            <div className="w-full md:w-72 h-[20vh] md:h-auto bg-gray-100 dark:bg-[#2c2c2c] p-4 border-t md:border-t-0 md:border-l border-[#eef6ef] dark:border-[#2c2c2c] overflow-y-auto">
              <h2 className="text-lg font-semibold text-[#1b281b] dark:text-white mb-4">Reminders</h2>
              {pendingReminders.size > 0 ? (
                Array.from(pendingReminders).map((taskId) => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        <p className="text-sm font-medium text-[#1b281b] dark:text-white">Reminder: {task.title}</p>
